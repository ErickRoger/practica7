...
<body id="main_body" >
	...
	<div id="form_container">
		...
		<h1><a>Registrar Venta</a></h1>
		 <form id="form_1075005" class="appnitro2"  method="post" action="registrar_venta2.php">
			...
			<ul >
				<?php
					...
					$con=conectarse();
					$result=$con->query("SELECT AUTO_INCREMENT FROM information_schema.TABLES WHERE TABLE_SCHEMA = 'pollos' AND TABLE_NAME = 'ventas'");
				...
				?>		
					...
					<span>
						<input id="resp" name= "resp" class="element text" maxlength="255" size="11" value="<?php echo $_SESSION['nomusuario']." ".$_SESSION['apeusuario']; ?>" required />		
				...
				<center><a href="almacen.php"><---</a></center>
					...
					<form class="appnitro"  method="post" action="registrar_venta.php"> 
					<tr bgcolor="#FF9025">
					<li id="li_1" >
				...
				<input id="saveForm" class="button_text" type="submit" name="submit" value="+" />
					...
					if(isset($_POST['submit']))
					{
						...
						$c=$con->query("SELECT * FROM tempdetallev WHERE codigo='$codigo'");
						//$rowcount=mysqli_num_rows($c);
						if($c->num_rows==1)
						{
							$con->query("UPDATE tempdetallev SET cant='$cant', subtotal='$sub' WHERE codigo='$codigo'");
						}
						else
						{
							$ins = $con->query("INSERT INTO tempdetallev(codigo, producto, cant, valorun, subtotal) 
					...
					?>
					<tr>
					<td colspan="5" align="right">Total: <?php  $s=$con->query("SELECT sum(subtotal) as suma FROM tempdetallev"); $r=$s->fetch_array();	echo $r['suma']; ?> </td>
					</tr>
					</table>	
					</center>	
					<?php
